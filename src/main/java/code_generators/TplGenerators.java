...
public class TplGenerators {
    public static void main(String... args) throws Exception {
        ...
        Arrays.asList(strings).forEach(nm -> {
            String fs = "package io.crm.util.touple.immutable;\n" +
                    ...
                    "\n" +
                    al(cc, "ps") +
                    "    public Tpl" + (cc.counter - 1) + "<" + ts((cc.counter - 1)) + "> pp() {\n" +
    ...
    private static String al(final SimpleCounter cc, String functionName) {
        if (cc.counter + 1 > 8) return "";
        return "    public <T" + (cc.counter + 1) + "> Tpl" + (cc.counter + 1) + "<" + ts(cc.counter) + ", T" + (cc.counter + 1) + "> " + functionName + "(final T" + (cc.counter + 1) + " val) {\n" +
